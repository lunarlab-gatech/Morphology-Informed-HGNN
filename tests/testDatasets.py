...
class TestFlexibleDatasets(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path_to_go2_urdf = Path('urdf_files', 'Go2', 'go2.urdf').absolute()
        ...
        self.dataset_go2_dynamics = QuadSDKDataset_Go2_Flat_Speed0_5_Mu_50(
            self.path_to_go2_seq, self.path_to_go2_urdf, 'package://go2_description/', 
            'unitree_ros/robots/go2_description', 'dynamics', 1, normalize=False)
    ...
    def test__init__(self):
        ...
        with self.assertRaises(ValueError):
            QuadSDKDataset_Go2_Flat_Speed0_5_Mu_50(self.path_to_go2_seq, self.path_to_go2_urdf, 
    ...
    def test_get_helper_dynamics(self):
        ...
        des_q = np.array([0.11106036, 0.0219779, 0.26230157,
                          -0.01051185, -0.02242382, 0.0031927, 0.99968819,  
                           0.00985092,  0.67495145,  1.34233534,
                          -0.00328921,  0.51851414,  1.03396776,
                          -0.0077084,   0.48193389,  0.98210022,
                           0.01678816,  0.64268708,  1.26193817], dtype=np.double)
        des_vel = np.array([ 0.28714938,  0.09367028, -0.01043991,
                             -0.18767554,  0.01491843,  0.25352565, 
        ...
        des_acc = np.array([-0.13492641,  0.39766349,   10.62749736,
                             5.59381084, -0.06683834,    0.10924432,
        ...
        des_labels = np.array([63.22432284, 0, 0, 71.78180083], dtype=np.double)

        # Test that the values are correct
        np.testing.assert_array_almost_equal(q.numpy(), des_q, 8)
        np.testing.assert_array_almost_equal(vel.numpy(), des_vel, 8)
        np.testing.assert_array_almost_equal(acc.numpy(), des_acc, 8)
